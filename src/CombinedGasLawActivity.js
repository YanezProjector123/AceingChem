...
function getRandomProblem() {
  ...
  let T1 = +(Math.random() * 200 + 100).toFixed(2); // K
  let P2 = +(Math.random() * 2 + 1).toFixed(2); // atm
  let V2 = +(Math.random() * 20 + 1).toFixed(2); // L
  let T2 = +(Math.random() * 200 + 100).toFixed(2); // K

  let question, answer, units;
  switch (solveFor) {
    case 'P1':
      answer = (P2 * V2 * T1) / (T2 * V1);
      question = `Given V1 = ${V1} L, T1 = ${T1} K, P2 = ${P2} atm, V2 = ${V2} L, T2 = ${T2} K, what is the initial pressure (P1) in atm?`;
      units = 'atm';
      break;
    case 'V1':
      answer = (P2 * V2 * T1) / (T2 * P1);
      question = `Given P1 = ${P1} atm, T1 = ${T1} K, P2 = ${P2} atm, V2 = ${V2} L, T2 = ${T2} K, what is the initial volume (V1) in L?`;
      units = 'L';
      break;
    case 'T1':
      answer = (T2 * P1 * V1) / (P2 * V2);
      question = `Given P1 = ${P1} atm, V1 = ${V1} L, P2 = ${P2} atm, V2 = ${V2} L, T2 = ${T2} K, what is the initial temperature (T1) in K?`;
      units = 'K';
      break;
    case 'P2':
      answer = (P1 * V1 * T2) / (T1 * V2);
      question = `Given P1 = ${P1} atm, V1 = ${V1} L, T1 = ${T1} K, V2 = ${V2} L, T2 = ${T2} K, what is the final pressure (P2) in atm?`;
      units = 'atm';
      break;
    case 'V2':
      answer = (P1 * V1 * T2) / (T1 * P2);
      question = `Given P1 = ${P1} atm, V1 = ${V1} L, T1 = ${T1} K, P2 = ${P2} atm, T2 = ${T2} K, what is the final volume (V2) in L?`;
      units = 'L';
      break;
    case 'T2':
      answer = (T1 * P2 * V2) / (P1 * V1);
      question = `Given P1 = ${P1} atm, V1 = ${V1} L, T1 = ${T1} K, P2 = ${P2} atm, V2 = ${V2} L, what is the final temperature (T2) in K?`;
      units = 'K';
      break;
    default:
      break;
